const publicUrl = "https://ohyagan-crypto.github.io/mn-dashboard-b1b9bd72f800/";
...
const dashboardStats = [
  { label: "已安裝項目", value: "169", note: "包含主技能、相容入口與歷史版本" },
  { label: "去重技能", value: "61", note: "目前可搜尋與執行的獨立能力" },
  { label: "工作記憶", value: "337", note: "規則、偏好、成功流程與工作交接記錄" },
...
if (Array.isArray(window.TG123_SKILLS) && window.TG123_SKILLS.length) {
  skillCatalog = window.TG123_SKILLS.map(([name, group, description]) => ({
    name,
    group,
    variants: 1,
...
function renderSkills() {
  ...
  const summaryItems = [
    { value: "169", label: "安裝項目", note: "含相容入口與歷史版本" },
    { value: String(skillCatalog.length), label: "去重技能", note: "目前可搜尋的獨立能力" },
    { value: String(groups.length - 1), label: "技能分類", note: "依工作用途整理" },
  ...
  setTextIfPresent("#chipSkillStatus", `169 安裝項目 / ${skillCatalog.length} 項去重技能`);
...
function setupQuickCommandCopy() {
  const buttons = document.querySelectorAll('[data-copy]');
  if (!buttons.length) return;
  const toast = document.createElement('div');
  toast.className = 'copy-toast';
  toast.textContent = '已複製指令';
